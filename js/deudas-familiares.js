/* Hogar Finanzas — Vista familiar de deudas y administración centralizada en el FAB */
(() => {
  'use strict';
  ...
  function vencimientoPrestamo(p = {}) {
    return fechaValida(p.proximoVencimiento || p.fechaVencimiento || p.vencimiento);
  }

  function miembroActual() {
    try {
      return typeof window.obtenerMiembroActual === 'function' ? window.obtenerMiembroActual() : null;
    } catch (_) {
      return null;
    }
  }
  ...
  function resumenTarjeta(tarjeta = {}) {
    ...
    if (linea > 0 && disponible < 0) {
      nivel = 'danger';
      etiqueta = 'Excedida';
      consejo = `Supera la línea por ${moneda(Math.abs(disponible))}. Conviene no usarla hasta reducir la deuda.`;
    } else if (dias !== null && dias < 0) {
      nivel = 'danger';
      etiqueta = 'Vencida';
      consejo = `El vencimiento fue hace ${Math.abs(dias)} día${Math.abs(dias) === 1 ? '' : 's'}. Revisa si el pago ya fue realizado.`;
    } else if ((dias !== null && dias <= 3) || uso >= 85) {
      nivel = 'danger';
      etiqueta = dias !== null && dias <= 3 ? 'Vence pronto' : 'Casi al límite';
      consejo = dias !== null && dias <= 3
        ? `Vence ${dias === 0 ? 'hoy' : `en ${dias} día${dias === 1 ? '' : 's'}`}.`
        : 'Queda muy poco crédito disponible; evita nuevas compras.';
    } else if ((dias !== null && dias <= 7) || uso >= 65) {
      nivel = 'watch';
      etiqueta = 'Atención';
      consejo = dias !== null && dias <= 7
        ? `El pago vence en ${dias} días.`
        : 'La tarjeta ya usa una parte importante de su línea.';
    }
  ...
  function resumenPrestamo(prestamo = {}) {
    ...
    if (dias !== null && dias < 0) {
      nivel = 'danger';
      etiqueta = 'Vencido';
      consejo = `La cuota venció hace ${Math.abs(dias)} día${Math.abs(dias) === 1 ? '' : 's'}.`;
    } else if (dias !== null && dias <= 3) {
      nivel = 'danger';
      etiqueta = 'Vence pronto';
      consejo = dias === 0 ? 'La cuota vence hoy.' : `La cuota vence en ${dias} días.`;
    } else if (dias !== null && dias <= 7) {
      nivel = 'watch';
      etiqueta = 'Próximo';
      consejo = `La cuota vence en ${dias} días.`;
    }

    return { prestamo, saldo, cuota, vencimiento, dias, nivel, etiqueta, consejo };
  }

  function claseEstado(nivel) {
    return nivel === 'danger' ? 'status-danger' : nivel === 'watch' ? 'status-watch' : '';
  }

  function tarjetaHTML(r) {
    const porcentaje = Math.max(0, Math.min(100, r.uso));
    const disponibleTexto = r.linea > 0
      ? (r.disponible < 0 ? `− ${moneda(Math.abs(r.disponible))}` : moneda(r.disponible))
      : 'No informada';
    ...
    return `
      <article class="hf-family-card ${claseEstado(r.nivel)}">
        <div class="hf-family-card-head">
          <span>
            <strong>${escapar(nombreTarjeta(r.tarjeta))}</strong>
            <small>${r.tarjeta.ultimosDigitos ? `Termina en ${escapar(r.tarjeta.ultimosDigitos)}` : 'Tarjeta de crédito'}</small>
          </span>
        ...
        ${r.linea > 0 ? `
          <div class="hf-family-credit-bar"><i style="width:${porcentaje.toFixed(1)}%"></i></div>
          <div class="hf-family-credit-labels"><span>${Math.round(r.uso)}% utilizado</span><span>Línea ${moneda(r.linea)}</span></div>
        ` : ''}
  ...
  function prestamoHTML(r) {
    ...
    return `
      <article class="hf-family-card hf-family-loan-card ${claseEstado(r.nivel)}">
        ...
        ${total > 0 ? `
          <div class="hf-family-credit-bar"><i style="width:${progreso.toFixed(1)}%"></i></div>
          <div class="hf-family-credit-labels"><span>${Math.round(progreso)}% pagado</span><span>Original ${moneda(total)}</span></div>
        ` : ''}
        <div class="hf-family-card-foot">
          <div><span>Vencimiento</span><strong>${fechaCorta(r.vencimiento)}</strong></div>
  ...
  function prioridadesHTML(tarjetas, prestamos) {
    ...
    tarjetas.forEach(r => {
      if (r.nivel === 'danger') {
        items.push({
          ...
          fecha:r.vencimiento,
          icono:'💳',
          nombre:nombreTarjeta(r.tarjeta),
          detalle:r.consejo,
          monto:r.minimo || r.deuda
        });
      } else if (r.dias !== null && r.dias <= 15) {
        items.push({
          prioridad:1,
          fecha:r.vencimiento,
          icono:'💳',
          nombre:nombreTarjeta(r.tarjeta),
          detalle:`Vence ${fechaCorta(r.vencimiento)}`,
          monto:r.minimo
        });
      }
    });
    ...
    prestamos.forEach(r => {
      if (r.nivel === 'danger' || (r.dias !== null && r.dias <= 15)) {
        items.push({
          prioridad:r.nivel === 'danger' ? 0 : 1,
          fecha:r.vencimiento,
          icono:'🏦',
          nombre:nombrePrestamo(r.prestamo),
          detalle:r.consejo,
          monto:r.cuota
        });
      }
    ...
    if (!items.length) {
      return '<div class="hf-family-priority-empty">No hay vencimientos cercanos ni tarjetas excedidas con la información registrada.</div>';
    }

    return `<div class="hf-family-priority-list">${items.slice(0,4).map(item => `
      <div class="hf-family-priority-item">
        <i>${item.icono}</i>
        <div><strong>${escapar(item.nombre)}</strong><small>${escapar(item.detalle)}</small></div>
        <b>${item.monto > 0 ? moneda(item.monto) : ''}</b>
      </div>`).join('')}</div>`;
  }

  function rutaHTML(deudaTotal, pagoMensual) {
    if (deudaTotal <= 0) {
      return '<section class="hf-family-route"><span>Ruta de deuda</span><strong>No hay deuda pendiente registrada.</strong><p>Las tarjetas y préstamos aparecerán aquí cuando tengan saldo.</p></section>';
    }
    if (pagoMensual <= 0) {
      return `<section class="hf-family-route"><span>Ruta de deuda</span><strong>Deben ${moneda(deudaTotal)} en total.</strong><p>Falta registrar pagos mínimos o cuotas para estimar un tiempo de pago comprensible.</p></section>`;
    }
    const meses = Math.ceil(deudaTotal / pagoMensual);
    const avance = Math.min(100, pagoMensual / deudaTotal * 100);
    return `<section class="hf-family-route">
      <span>Referencia para todas las deudas</span>
      <strong>Entre mínimos y cuotas tienen registrados ${moneda(pagoMensual)} al mes.</strong>
      <p>La deuda actual equivale aproximadamente a ${meses} meses de esos pagos, sin nuevas compras ni intereses futuros.</p>
      <div class="hf-family-route-bar"><i style="width:${avance.toFixed(1)}%"></i></div>
      <small>Es una referencia, no una fecha exacta.</small>
    </section>`;
  }
  ...
  function inyectarAdminModal() {
      ...
      <div class="modal-overlay" id="hfDebtAdminModal" onclick="closeModalOutside(event,'hfDebtAdminModal')">
        <div class="modal-sheet hf-app-sheet hf-debt-admin-sheet" style="position:relative">
          ...
          <div class="hf-debt-admin-heading">
            ...
            <div><div class="modal-title">Administrar deudas</div><p>Actualiza datos, registra pagos y usa las herramientas de decisión.</p></div>
          </div>
          <div class="hf-debt-admin-actions">
            <button class="hf-debt-admin-action" type="button" data-admin-action="actualizar">
              <span>↻</span><div><strong>Actualizar información bancaria</strong><small>Saldos, pago mínimo y vencimiento.</small></div><em>›</em>
            </button>
            <button class="hf-debt-admin-action" type="button" data-admin-action="cierre">
              <span>▣</span><div><strong>Guardar historial del mes</strong><small>Conserva una fotografía para comparar la deuda.</small></div><em>›</em>
            </button>
            <button class="hf-debt-admin-action" type="button" data-admin-action="plan">
              <span>📉</span><div><strong>Calcular cómo pagar</strong><small>Simula cuotas, tiempo e intereses.</small></div><em>›</em>
            </button>
            <button class="hf-debt-admin-action" type="button" data-admin-action="tarjeta">
              <span>💳</span><div><strong>Agregar tarjeta</strong><small>Registra una nueva línea de crédito.</small></div><em>›</em>
            </button>
            <button class="hf-debt-admin-action" type="button" data-admin-action="prestamo">
              <span>🏦</span><div><strong>Agregar préstamo</strong><small>Registra saldo, cuota y vencimiento.</small></div><em>›</em>
            </button>
          </div>
    ...
    document.querySelectorAll('#hfDebtAdminModal [data-admin-action]').forEach(boton => {
      boton.addEventListener('click', () => ejecutarAccionAdmin(boton.dataset.adminAction));
    });

    if (!estado.modalObserver) {
      estado.modalObserver = new MutationObserver(() => setTimeout(aplicarFabAdministracion, 80));
      estado.modalObserver.observe($('hfDebtAdminModal'), { attributes:true, attributeFilter:['class'] });
    }
  }
  ...
  function renderAdminPagos(tarjetas) {
    ...
    contenedor.innerHTML = tarjetas.map(r => `
      <div class="hf-admin-card-row" data-admin-card-id="${escapar(r.tarjeta.id)}">
        <div><strong>${escapar(nombreTarjeta(r.tarjeta))}</strong><small>Deuda ${moneda(r.deuda)}</small></div>
        <button type="button">Registrar pago</button>
      </div>`).join('');

    contenedor.querySelectorAll('[data-admin-card-id]').forEach(fila => {
  ...
  function instalarFab() {
    const fab = $('fab-global');
    if (fab && !estado.fabListener) {
      estado.fabListener = true;
      fab.addEventListener('click', event => {
        if (!paginaDeudasActiva()) return;
        event.preventDefault();
        event.stopImmediatePropagation();
        if (esAdministrador()) abrirAdministracion();
      }, true);
    }

    if (!estado.documentListener) {
      estado.documentListener = true;
      document.addEventListener('click', () => {
        if (paginaDeudasActiva()) setTimeout(aplicarFabAdministracion, 120);
      });
    }
  ...
  async function renderizar() {
    ...
    try {
      ...
      vista.innerHTML = `
        <div class="hf-family-debt-head">
          <div><h2>Así están nuestras deudas</h2><p>Una vista sencilla para saber cuánto debemos, cuánto crédito queda y qué pagos se acercan.</p></div>
        </div>
  ...
  function iniciar() {
    ...
    ['hf:deudas-core-actualizadas','hf:deuda-actualizada','hf:estado-cuenta-confirmado','hf:cierre-mensual-guardado','hf:gastos-actualizados']
    ...
    if (pagina && !estado.observer) {
      estado.observer = new MutationObserver(mutations => {
        if (mutations.some(m => m.type === 'attributes')) {
          setTimeout(aplicarFabAdministracion, 50);
        } else {
          programar();
        }
      });
      estado.observer.observe(pagina, { attributes:true, attributeFilter:['class'] });
      ['tarjetas-grid','prestamos-grid'].forEach(id => {
        const nodo = $(id);
        if (nodo) estado.observer.observe(nodo, { childList:true });
      });
    }
  }
  ...
  function obtenerEstado() {
    return {
      ...
      vistaDisponible:Boolean($('hf-family-debt-view')),
      admin:estado.admin,
      modalAdminDisponible:Boolean($('hfDebtAdminModal')),
      tarjetas:estado.tarjetas.length,
      prestamos:estado.prestamos.length,
      fabIntegrado:estado.fabIntegrado,
      botonAdministrarVisible:Boolean(document.querySelector('#hf-family-debt-view .hf-family-admin-button'))
    };
  }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setTimeout(iniciar, 300), { once:true });
  } else {
    setTimeout(iniciar, 160);
  }
})();