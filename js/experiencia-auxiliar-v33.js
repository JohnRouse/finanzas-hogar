(() => {
  ...
  const VERSION = '33.0';
  if (window.HFExperienciaAuxiliar33?.version === VERSION) return;
  ...
  function normalizePaymentModal() {
    ...
    const inputs = [...modal.querySelectorAll('#pago-disponible-banco')];
    if (!inputs.length) return;

    const preferred = inputs.find(input => input.closest('.hf-v27-payment-available, .hf-payment-bank-balance')) || inputs.at(-1);
  ...
  function unifiedFabHandler() {
    if ($('page-gastos')?.classList.contains('active')) {
  ...
  function ensureHomeProfile() {
    const grid = $('page-resumen')?.querySelector('.month-money-grid');
    if (!grid) return null;
  ...
  async function updateAvatarUI() {
    try {
      ...
      const settingsProfile = $('aj-perfil-avatar');
  ...
  function handleClick(event) {
    if (event.target.closest('[data-hf-open-avatar]')) {
  ...
  window.HFExperienciaAuxiliar33 = Object.freeze({
    version: VERSION,
    updateAvatarUI,
    normalizePaymentModal,
    removeDebtQuickPayments
