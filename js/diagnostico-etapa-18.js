...
(() => {
  ...
  async function ejecutar() {
    const engine = window.HFEtapa18Beta5;
    if (!engine || !window.DB) throw new Error('La Etapa 18 beta 5 todavía no está lista.');

    await engine.reconcileSavingsLedger?.();
    await engine.refreshDebtStates?.();
    const context = await engine.financialContext?.();
    ...
    console.group('Hogar Finanzas · Diagnóstico Etapa 18 beta 5');
    ...
    console.table(goalRows.map(item => ({ meta:item.nombre, reservado:item.reservado, apartadoMes:item.apartadoMes, tracked:item.tracked, diferencia:item.diferencia })));
